...
sql, args, err := builder.NewSelect().
	From(table).
	Column(builder.ColumnCount{Alias: "a1"}).
	Column(builder.ColumnCount{
		Alias: "a2",
...
Subqueries created with `NewTableSub` can also be used as join targets:

```go
sub := builder.NewTableSub(subQuery)

q.InnerJoin(sub, builder.OnEq{
	Table1:  table1,
	Column1: "id",
	Table2:  sub,
	Column2: "table_id",
...
Bind arguments generated by the joined subquery are included in the outer query bind map.
